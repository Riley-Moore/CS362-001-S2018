			...
			for (int i = 0; i < NUM_TESTS; i++) {
					String methodName = ApptRandomTest.RandomSelectMethod(random);
					   if (methodName.equals("setTitle")){
						   ...
						   int recurday = ValuesGenerator.getRandomIntBetween(random,0,1);
						   if (recurday==1)
							 ...
							 int onDay=ValuesGenerator.getRandomIntBetween(random, -1, 32);
							 int onMonth=ValuesGenerator.getRandomIntBetween(random, -1, 13);
							 int onYear=ValuesGenerator.getRandomIntBetween(random, -1, 2018);
							 appt.isOn(onDay, onMonth, onYear);
